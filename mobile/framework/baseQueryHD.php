...
class QueryBaseHD extends Base{
	public $con;
	private $arrKey;
	private $dtlKey;	
	...
	public function setAttributeDTL($attrDTL) {
		$this->attrDTL = $attrDTL;
	}
	
	public function setPositionDTL($arrPosition) {
		$this->arrPositionDTL = $arrPosition;
	}
	
	public function setMaskDTL($arrMask) {
		$this->arrMaskDTL = $arrMask;
	}
	
	public function setQuoteDTL($arrQuote) {
		$this->arrQuoteDTL = $arrQuote;
	}
...
//Response
	...
	private function generateJSONHDR($newRow, $valueDTL) {
		...
		$retStr .= "," . $this->escquote($this->attrDTL, true) . ":[" . $valueDTL . "]";
	...
	private function generateJSONDTL($newRow) {
		...
		for ($i=0; $i<sizeof($this->arrPositionDTL);$i++) {
			$element = $this->escquote($this->arrMaskDTL[$i], true) . ":" . $this->escquote($newRow[$this->arrPositionDTL[$i]], $this->arrQuoteDTL[$i]);
			if ($i < 1)		
				$retStr .= $element;
			else
				$retStr .= "," . $element;
...
//Query
 ...
 /*
$con = openConnection();
$queryBaseHD = new QueryBaseHD($con, "select a.nodoc, a.tanggal, a.keterangan, a.total, a.creatime, b.kode, b.keterangan, b.qty, b.harga, b.jumlah, b.id from testheader a inner join testdetails b on a.nodoc=b.nodoc;");
$queryBaseHD->setKey(array(0), 5); //Header, Detail
...
try {	
	...
	echo $queryBaseHD->responseSucceed($retJSON);
}
catch (Exception $e) {
	echo $queryBaseHD->responseFailed("Failed", $e->getMessage());	
}
*/
/*
$con = openConnection();
